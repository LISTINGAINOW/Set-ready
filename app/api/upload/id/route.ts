...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (user) {
      // Check booking ownership (in-memory bookings keyed by bookingId prefix format)
      // We do a best-effort check: if the booking exists in Supabase, verify ownership
      ...
      if (booking && booking.guest_email !== user.email) {
        writeAuditLog('upload.id.forbidden', { ip, bookingId, userId });
    ...
    writeAuditLog('upload.id.success', { ip, bookingId, path, userId });
